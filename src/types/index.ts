...
import { Camera, Intersection, Object3D, Object3DEventMap, Quaternion, Raycaster, Vector2, Vector3 } from "three";
...
export type ContainerAction =
  ...
  | { type: 'REMOVE_PIECE'; pieceId: number; onResult?: (shape: PieceShape | null) => void }
...
export type ContainerControl = {
  getState: () => RefObject<ContainerState>;
  getPosition: () => ReadonlyVector3Tuple;
  getCenteredPosition: () => ReadonlyVector3Tuple;
...
export type DragHandler = (
  state: DragState,
  targetVec: Vector3,
  { camera, raycaster, pointer, hits, button, deltaX, deltaY, downInfo}: 
    {camera: Camera, raycaster: Raycaster, pointer: Vector2, hits: Intersection[], button: number, deltaX: number, deltaY: number, downInfo: DownInfo}
) => void;
...
export type RemovePieceCallback = {
  shape: PieceShape;
  center: Vector3;
}