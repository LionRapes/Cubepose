...
import type { DragHandler, DragPredicate, PointerCallback, PointerHandler, ReadonlyVector3Tuple } from '../types';
...
export function useDrag({
  ...
  const handleMove = useCallback<PointerHandler>((event, hits, extra) => {
    ...
    onMoveRef.current?.(
      dragState,
      targetVec.current,
      {...extra, hits, button: event.buttons, deltaX: event.movementX, deltaY: event.movementY});
  }, []);

  const handleDown = useCallback<PointerHandler>((event, hits, extra) => {
    if (!onDownPredicateRef.current?.(dragState, targetVec.current, {...extra, hits, button: event.buttons})) return; 

    onDownRef.current?.(
      dragState,
      targetVec.current,
      {...extra, hits, button: event.button, deltaX: event.movementX, deltaY: event.movementY});
  }, []);
